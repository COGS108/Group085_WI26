...
def get_gdrive(file_list, destination_directory):
    """
    Downloads a list of files from Google Drive shareable links to a specified destination directory with progress bars.
    ...
    for file_info in tqdm(file_list, desc="Overall Download Progress"):
        ...
        try:
            # learned how to use gdown from the official repository: https://github.com/wkentaro/gdown
            gdown.download(url, local_filepath)
            print(f"Successfully downloaded: {filename}")
        except requests.exceptions.RequestException as e:
            print(f"Error downloading {filename}: {e}")
        except Exception as e:
